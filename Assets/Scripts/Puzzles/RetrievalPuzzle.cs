...
public class RetrievalPuzzle : Interactable
{
    // Notes from Jackson:
    ...
    public List<DialogStructRetrieval> talkAgainList = new List<DialogStructRetrieval>();

    private List<List<DialogStructRetrieval>> responseList = new List<List<DialogStructRetrieval>>();

    private List<DialogStructRetrieval> refDialogueList = new List<DialogStructRetrieval>();
    ...
    public void setList()
    {
        refDialogueList = DialogueManager.GetComponent<ReadDialogueRetrieval>().DialogList;
        foreach (DialogStructRetrieval DialogueRow in refDialogueList)
        {
            if (DialogueRow.scene.Contains(scene) == true)
            {
                if (DialogueRow.talkAgain == false)
                {
                    DialogList.Add(DialogueRow);
                }
                else
                {
    ...
    public override void Interact()
    {
        ...
        // set first line
        DialogueManager.GetComponent<ReadDialogueRetrieval>().RetrievalPuzzle = this.gameObject;
        setDialogueUI();
    }
    ...
    public override void Finished()
    {
        ...
        if (increasePriority)
        {
            DialogueManager.GetComponent<ReadDialogueRetrieval>().priority++;
            DialogueManager.GetComponent<ReadDialogueRetrieval>().setDialogueTools();
        }
    ...
    public bool setOptions()
    {
        ...
        if (talkAgain)
        {
            ...
            if (talkAgainList[index].options)
            {
                ...
                for (int i = 1; i <= talkAgainList[index].optionNumber; i++)
                {
                    ...
                    newButton.GetComponent<OptionButtonSetUpRetrieval>().optionNumber = i;
                    buttonList.Add(newButton);
                    List<DialogStructRetrieval> optionList = new List<DialogStructRetrieval>();
                    foreach (DialogStructRetrieval optionDialog in talkAgainList)
                    {
                    ...
                    newButton.GetComponent<OptionButtonSetUpRetrieval>().OptionResponseList = optionList;
                    newButton.GetComponent<OptionButtonSetUpRetrieval>().originalRetrievaPuzzle = this.gameObject;
        ...
        else
        {
            ...
            if (DialogList[index].options)
            {
                refDialogueList = DialogList;
                hadOption = true;
                setProfile();
                setName();
                setLine();
                for (int i = 1; i <= DialogList[index].optionNumber; i++)
                {
                    GameObject newButton = Instantiate(optionButtonPrefab, Options.transform);
                    newButton.GetComponent<OptionButtonSetUpRetrieval>().optionNumber = i;
                    buttonList.Add(newButton);
                    List<DialogStructRetrieval> optionList = new List<DialogStructRetrieval>();
                    foreach (DialogStructRetrieval optionDialog in DialogList)
                    {
                        if (optionDialog.optionNumber == i && optionDialog.scene.Contains("Response") == true)
                        {
                            optionList.Add(optionDialog);
    ...
    public void setProfile()
    {
        if (talkAgain)
        {
            switch (talkAgainList[index].align)
            {
                case Alignment.Left:
                    ...
                    Profile.sprite = DialogueManager.GetComponent<ReadDialogueRetrieval>().ProfileImages[talkAgainList[index].profileNumber - 1];
                    break;

                case Alignment.Right:
                    Profile.gameObject.SetActive(false);
                    Profile2.gameObject.SetActive(true);
                    Profile2.sprite = DialogueManager.GetComponent<ReadDialogueRetrieval>().ProfileImages[talkAgainList[index].profileNumber - 1];
                    break;
        ...
        {
            switch (DialogList[index].align)
            {
                case Alignment.Left:
                    Profile.gameObject.SetActive(true);
                    Profile2.gameObject.SetActive(false);
                    Profile.sprite = DialogueManager.GetComponent<ReadDialogueRetrieval>().ProfileImages[DialogList[index].profileNumber - 1];
                    break;

                case Alignment.Right:
                    Profile.gameObject.SetActive(false);
                    Profile2.gameObject.SetActive(true);
                    Profile2.sprite = DialogueManager.GetComponent<ReadDialogueRetrieval>().ProfileImages[DialogList[index].profileNumber - 1];
                    break;
    ...
    public void setName()
    {
        ...
        {
            Name.text = DialogList[index].name;
            switch (DialogList[index].align)
            {
                case Alignment.Left:
                    Name.alignment = TextAlignmentOptions.Left;
    ...
    public void setLine()
    {
        ...
        {
            Dialogue.text = DialogList[index].dialogue;
            switch (DialogList[index].fontStyle)
            {
                case FontSelectStyle.Normal:
                    Dialogue.fontStyle = FontStyles.Normal;
                    break;
    ...
    public void setSelectedOptionDialogue(GameObject selectedButton)
    {
        ...
        inOptionDialog = true;
        DialogList = responseList[selectedButton.GetComponent<OptionButtonSetUp>().optionNumber - 1];
